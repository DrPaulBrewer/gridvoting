...
class MarkovChainGPU():
    def __init__(self, *, P, computeNow=True, tolerance=1e-10):
        ...
        self.has_unique_stationary_distibution = \
            not xp.any(self.absorbing_points)
...
class VotingModel():
    ...
    def analyze(self):
        ...
        if not self.core_exists:
            self.stationary_distribution = \
                xp.asnumpy(self.MarkovChain.stationary_distribution)
        self.analyzed = True
    ...
    def _get_transition_matrix(self):
        ...
        if zi:
            cP = xp.divide(
                xp.add(
                    cV,
                    xp.diag(xp.subtract(nfa, cV_sum_of_row))
                ),
                nfa
            )
        else:
            cP = xp.divide(
                xp.add(cV, xp.eye(nfa)),
                (1+cV_sum_of_row)[:, xp.newaxis]
            )
